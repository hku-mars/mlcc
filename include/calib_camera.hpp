...
public:
  ...
  void loadImgAndPointcloud()
  {
    ...
    for(size_t i = 0; i < pose_size; i++)
    {
      ...
      pcl::io::loadPCDFile(lidar_path+to_string(base_number)+"/"
                           +to_string(i)+".pcd", *pc_base);
    ...
    for(int i = 0; i < total_ext_number; i++)
    {
      ...
      for(size_t j = 0; j < pose_size; j++)
      {
        ...
        pcl::io::loadPCDFile(lidar_path+to_string(ext_lidars[i].lidar_number)+"/"
                             +to_string(j)+".pcd", *pc_ref);
                   ...
                   std::vector<Eigen::Vector3d>& line_point)
  {
    ...
    for(int round = 0; round < 2; round++)
    {
      ...
      double D =
        -(A * projection_center[0] + B * projection_center[1] + C * projection_center[2]);
      ...
      double cx = x_axis[2];
      double dx =
        -(ax * projection_center[0] + bx * projection_center[1] + cx * projection_center[2]);
      double ay = y_axis[0];
      double by = y_axis[1];
      double cy = y_axis[2];
      double dy =
        -(ay * projection_center[0] + by * projection_center[1] + cy * projection_center[2]);
